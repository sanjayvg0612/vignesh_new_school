...
export default function SliderPage() {
  ...
  const fetchBanners = useCallback(async () => {
    ...
    try {
      const res    = await bannerApi.list({ page, page_size: PAGE_SIZE })
      const result = res.result || res
      setData(Array.isArray(result.banners) ? result.banners : Array.isArray(result.data) ? result.data : Array.isArray(result) ? result : [])
  ...
  const handleUpload = async () => {
    ...
    setSaving(true)
    try {
      await bannerApi.create(files, status)
      setModal(false)
      setPage(1)
      fetchBanners()
  ...
  const toggleStatus = async (item) => {
    const id        = item.banner_id ?? item.id
    const newStatus = item.status === 1 ? 0 : 1
    try {
      await bannerApi.update(id, { status: newStatus })
      setData(d => d.map(r => (r.banner_id ?? r.id) === id ? { ...r, status: newStatus } : r))
    } catch (e) { alert(e.message) }
  }
  ...
  const imgUrl = (item) => {
    const id = item.banner_id ?? item.id
    return item.image_url || item.file_url || bannerApi.imageUrl(id)
  }
    ...
    <div>
      <PageHeader
        ...
        action={
          <button onClick={openUpload} className="btn-primary flex items-center gap-2">
            <Plus className="w-4 h-4" /> Upload Banners
          </button>
        }
      />
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-gray-400 text-sm">No banners yet. Click Upload Banners to add images.</p>
        </div>
      ) : (
        <>
                  ...
                  <div className="relative h-48 bg-gray-100 overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                      <button
                        ...
                        className="p-2.5 rounded-full bg-white/90 hover:bg-white text-gray-700"
                      >
                      ...
                      <button
                        ...
                        title="Delete"
                        className="p-2.5 rounded-full bg-white/90 hover:bg-white text-red-500"
                      >
          ...
          {previews.length > 0 && (
            <div className="grid grid-cols-3 gap-2 max-h-64 overflow-y-auto">
              {previews.map((src, idx) => (
                <div key={idx} className="relative rounded-lg overflow-hidden h-24 bg-gray-100">
      ...
      >
        <p className="text-sm text-gray-600">Are you sure you want to delete this banner? This action cannot be undone.</p>
      </Modal>
    </div>
  )
}
