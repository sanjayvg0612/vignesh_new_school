...
export default function AllStaffPage() {
  ...
  const openModal = async (item) => {
    ...
    setForm({
      ...
      status:        item.status        || 'non teaching',
  ...
  const handleDelete = async (item) => {
    ...
    try { await employeeApi.delete(item.emp_db_id ?? item.id); fetchStaff() }
    catch (e) { alert(e.message) }
  }
  ...
  return (
    <div>
      <PageHeader
        title="All Staff"
        subtitle="Manage non-teaching and support staff"
        action={<Link href="/admin/staff/add" className="btn-primary">+ Add Staff</Link>}
      />
      ...
      >
        ...
        <FormField label="Qualification">
          <input className="input" value={form.qualification} onChange={e => setForm(f => ({ ...f, qualification: e.target.value }))} placeholder="e.g. B.Com, MBA" />
        </FormField>
        <div className="grid grid-cols-2 gap-3">
