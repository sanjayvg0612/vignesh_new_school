    ...
    <div>
      ...
      {success && (
        <div className="mb-4 flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-sm rounded-lg px-4 py-3">
          <CheckCircle className="w-4 h-4 shrink-0" /> Staff added successfully! Redirecting...
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Qualification">
              <input className="input" value={form.qualification} onChange={f('qualification')} placeholder="e.g. B.Com, MBA" />
            </FormField>
            <FormField label="Salary">
              <input className="input" type="number" value={form.salary} onChange={f('salary')} placeholder="e.g. 20000" />
            </FormField>
          </div>
          <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button type="button" onClick={() => router.push('/admin/staff')} className="btn-secondary">Cancel</button>
            <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Add Staff'}</button>
          </div>
        </form>
      </div>
    </div>
